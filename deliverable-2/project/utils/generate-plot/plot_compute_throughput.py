...
for impl in valid_impls:
    ...
    for f in csv_files:
        try:
            ...
            df['compute throughput'] = df['Compute Throughput [%]'].str.replace(
                ',', '.').astype(float)
...
for impl, df in mat_df.items():
    ...
    total_compute_throughput = df_filtered.groupby(
        'matrix')['compute throughput'].mean()

    for matrix in all_matrices:
        throughput = total_compute_throughput.get(matrix)
...
# ax.set_yscale('log')
ax.set_ylabel('Compute Throughput (%)', fontsize=14)
ax.set_xlabel('Input Matrix', fontsize=14)
ax.set_title('SpMV Compute Throughput', fontsize=16)
ax.set_xticks(x)
